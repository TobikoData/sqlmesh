...
export function PlanWizard({ id }: { id: string }): JSX.Element {
  ...
  function getContext(e: React.FormEvent<HTMLFormElement>): void {
    e.preventDefault()
    e.stopPropagation()
                        ...
                        >
                          <div className="flex justify-between items-center w-full">
                              ...
                              <p
                                className={clsx(
                                  ...
                                  changes?.modified?.direct?.find(
                                    change => change.model_name === snapshot.model_name
                                  ) != null && 'text-secondary-500',
                                  changes?.added?.find(
                                    modelName => modelName === snapshot.model_name
                            ...
                            <p className="inline-block text-xs">
                              <small>
                                {activePlan?.tasks?.[snapshot.model_name]?.completed ?? 0} /{' '}
                                {snapshot.batches} batch{snapshot.batches > 1 ? 'es' : ''}
                              </small>
                              <small className="inline-block ml-2 font-bold">
                                {Math.ceil(
                                  toRatio(
                                    activePlan?.tasks?.[snapshot.model_name]?.completed,
...
interface PropsPlanWizardStepHeader extends React.ButtonHTMLAttributes<HTMLElement> {
  headline: number | string
  disabled?: boolean
}
...
function PlanWizardStepHeader({
  ...
  return (
    <div className={clsx(disabled && 'opacity-40 cursor-not-allowed', 'mb-4 ', className)}>
      ...
      {children != null && <small className="text-gray-500">{children}</small>}
    </div>
  )
}
