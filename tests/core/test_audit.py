...
def test_macro(model: Model):
    expected_query = "SELECT * FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01') WHERE a IS NULL"
...
def test_not_null_audit(model: Model):
    ...
    assert (
        ...
        == "SELECT * FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01') WHERE a IS NULL"
    )
    ...
    assert (
        ...
        == "SELECT * FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01') WHERE a IS NULL OR b IS NULL"
    )
...
def test_unique_values_audit(model: Model):
    ...
    assert (
        rendered_query_a.sql()
        == "SELECT * FROM (SELECT ROW_NUMBER() OVER (PARTITION BY a ORDER BY 1) AS a_rank FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01')) WHERE a_rank > 1"
    )
    ...
    assert (
        rendered_query_a_and_b.sql()
        == "SELECT * FROM (SELECT ROW_NUMBER() OVER (PARTITION BY a ORDER BY 1) AS a_rank, ROW_NUMBER() OVER (PARTITION BY b ORDER BY 1) AS b_rank FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01')) WHERE a_rank > 1 OR b_rank > 1"
    )
...
def test_accepted_values_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == "SELECT * FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01') WHERE NOT a IN ('value_a', 'value_b')"
    )
...
def test_number_of_rows_audit(model: Model):
    ...
    assert (
        rendered_query.sql()
        == "SELECT 1 FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01') HAVING COUNT(*) <= 0 LIMIT 0 + 1"
    )
...
def test_forall_audit(model: Model):
    ...
    assert (
        rendered_query_a.sql()
        == "SELECT * FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01') WHERE NOT (a >= b)"
    )
    ...
    assert (
        rendered_query_a.sql()
        == "SELECT * FROM (SELECT * FROM db.test_model WHERE ds BETWEEN '1970-01-01' AND '1970-01-01') WHERE NOT (a >= b) OR NOT (c + d - e < 1.0)"
    )
