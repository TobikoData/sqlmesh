...
from sqlmesh.core.model import SeedKind, SeedModel, SqlModel, IncrementalByTimeRangeKind
...
def test_forward_only_dev(make_snapshot, mocker: MockerFixture):
    snapshot_a = make_snapshot(SqlModel(name="a", query=parse_one("select 1, ds"), kind=IncrementalByTimeRangeKind()))
