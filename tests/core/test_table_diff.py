...
def test_generated_sql(sushi_context_fixed_date: Context, mocker: MockerFixture):
    ...
    summary_query_sql = 'SELECT SUM("s_exists") AS "s_count", SUM("t_exists") AS "t_count", SUM("row_joined") AS "join_count", SUM("null_grain") AS "null_grain_count", SUM("row_full_match") AS "full_match_count", SUM("key_matches") AS "key_matches", SUM("value_matches") AS "value_matches", COUNT(DISTINCT ("s__key")) AS "distinct_count_s", COUNT(DISTINCT ("t__key")) AS "distinct_count_t" FROM memory."sqlmesh_temp"."__temp_diff_abcdefgh"'
    compare_sql = 'SELECT ROUND(100 * (SUM("key_matches") / COUNT("key_matches")), 1) AS "key_matches", ROUND(100 * (SUM("value_matches") / COUNT("value_matches")), 1) AS "value_matches" FROM memory."sqlmesh_temp"."__temp_diff_abcdefgh" WHERE "row_joined" = 1'
    sample_query_sql = 'SELECT "s_exists", "t_exists", "row_joined", "row_full_match", "s__key", "s__value", "t__key", "t__value" FROM memory."sqlmesh_temp"."__temp_diff_abcdefgh" WHERE "key_matches" = 0 OR "value_matches" = 0 ORDER BY "s__key" NULLS FIRST, "t__key" NULLS FIRST LIMIT 20'
