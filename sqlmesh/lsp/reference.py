...
from sqlmesh.core.linter.rules.helpers.lineage import (
...
def by_position(position: Position) -> t.Callable[[Reference], bool]:
    ...
    def contains_position(r: Reference) -> bool:
        return _position_within_range(position, to_lsp_range(r.range))
...
def get_model_find_all_references(
    ...
    model_at_position = next(
        filter(
            ...
            and _position_within_range(position, to_lsp_range(ref.range)),
    ...
    current_file_refs = filter(
        lambda ref: isinstance(ref, LSPModelReference) and ref.path == path,
        get_model_definitions_for_a_path(lint_context, document_uri),
    )
