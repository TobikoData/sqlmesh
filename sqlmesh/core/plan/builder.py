...
class PlanBuilder:
    ...
    def _check_destructive_changes(
        ...
        def _raise_or_warn(snapshot: Snapshot, destructive_model_name: t.Optional[str]) -> None:
            warning_msg = f"Changes to model '{destructive_model_name}' may result in a destructive change to forward-only model '{snapshot.name}'s schema."
            if snapshot.model.on_destructive_change.is_warn:
                logger.warning(warning_msg)
                return
            raise PlanError(
                f"{warning_msg} If the change is not destructive or to allow a destructive change, change the model's `on_destructive_change` setting to `warn` or `allow` or include it in the plan's `--allow-destructive-model` option."
            )

        snapshot_is_destructive: t.Dict[SnapshotId, t.Tuple[bool, t.Optional[str]]] = {}
        for s_id in dag:
            ...
            if s_id.name in self._context_diff.modified_snapshots:
                ...
                if snapshot and snapshot.is_model:
                    ...
                    snapshot_is_destructive[s_id] = (False, None)

                    # get parent classifications
                    for parent_id in snapshot.parents:
                        parent_is_destructive, parent_name = snapshot_is_destructive.get(
                            parent_id, (False, None)
                        )
                        if parent_is_destructive:
                            snapshot_is_destructive[s_id] = (True, parent_name)
                            if should_raise_or_warn:
                                _raise_or_warn(snapshot, parent_name)
                            break

                    # examine directly modified snapshots unless we already know the snapshot is destructive
                    if s_id in directly_modified and not snapshot_is_destructive[s_id][0]:
                        ...
                        if not columns_to_types_all_known(
                            ...
                            if has_drop:
                                snapshot_is_destructive[s_id] = (has_drop, snapshot.name)
                                if should_raise_or_warn:
                                    _raise_or_warn(snapshot, snapshot.name)
