    ...
    def _get_data_objects(
        ...
        for schema_val, name_val, type_val in rows:
            try:
                ...
                obj_type = str(type_val) if type_val is not None else "table"
                ...
                logger.error(f"Error processing row: {e}, row: {(schema_val, name_val, type_val)}")
                continue
    ...
    def _create_materialized_view(
        ...
        **create_kwargs: t.Any,
        ...
        with source_queries[0] as query:
            ...
            # 2) DISTRIBUTED BY and generic PROPERTIES (reuse builder on a filtered set)
            filtered_view_props: t.Dict[str, t.Any] = {}
            if view_properties:
                for k, v in view_properties.items():
                    if k in {"unique_key", "duplicate_key", "build", "refresh", "refresh_trigger"}:
                        continue
                    filtered_view_props[k] = v

            extra_props_node = self._build_table_properties_exp(
                catalog_name=target_table.catalog,
                table_properties=filtered_view_props or None,
                target_columns_to_types=target_columns_to_types,
            )
            if extra_props_node is not None and getattr(extra_props_node, "expressions", None):
                props.extend(extra_props_node.expressions)
            ...
            # Insert Doris-only clauses (BUILD / REFRESH / refresh_trigger / KEY / COMMENT / PARTITION BY) immediately after the column list
            ...
            if view_properties:
                ...
                if (uk := view_properties.get("unique_key")) is not None:
                    if isinstance(uk, exp.Tuple):
                        cols = [
                            c.sql(dialect=self.dialect, identify=True)
                            if isinstance(c, exp.Expression)
                            else exp.to_column(str(c)).sql(dialect=self.dialect, identify=True)
                            for c in uk.expressions
                        ]
                    else:
                        col_exp = uk if isinstance(uk, exp.Expression) else exp.to_column(str(uk))
                        cols = [col_exp.sql(dialect=self.dialect, identify=True)]
                    doris_inline_clauses.append(f"KEY ({', '.join(cols)})")
                elif (dk := view_properties.get("duplicate_key")) is not None:
                    if isinstance(dk, exp.Tuple):
                        cols = [
                            c.sql(dialect=self.dialect, identify=True)
                            if isinstance(c, exp.Expression)
                            else exp.to_column(str(c)).sql(dialect=self.dialect, identify=True)
                            for c in dk.expressions
                        ]
                    else:
                        col_exp = dk if isinstance(dk, exp.Expression) else exp.to_column(str(dk))
                        cols = [col_exp.sql(dialect=self.dialect, identify=True)]
            ...
            if partitioned_by:
                part_cols = ", ".join(
                    [
                        (
                            col.sql(dialect=self.dialect, identify=True)
            ...
            if doris_inline_clauses:
                ...
                # Find the end of the column list if present; otherwise, after the view name
                # Locate the first '(' after CREATE MATERIALIZED VIEW <name>
                    ...
                    for i in range(paren_start, len(create_sql)):
                        ...
                        if ch == "(":
                            ...
                            if depth == 0:
                                ...
                                break
                if insert_pos == -1:
                    # fallback: insert after the view name token
                    after_mv = create_sql.find("CREATE MATERIALIZED VIEW")
                    if after_mv != -1:
                        # find the space after the view identifier
                        # naive approach: find " AS " and insert before preceding space
