...
class _Model(ModelMeta, frozen=True):
    ...
    def render_query_with_original_sql(
        self,
        *,
        start: t.Optional[TimeLike] = None,
        end: t.Optional[TimeLike] = None,
        ...
        **kwargs: t.Any,
        ...
        Returns:
            The rendered query along with the original SQL, or None if the query can't be rendered.
        """
        query = exp.select(
            *(
        ...
        return d.RenderedQuery(rendered=query, original=None)

    def render_query_or_raise_with_original_sql(
        self,
        *,
        start: t.Optional[TimeLike] = None,
        end: t.Optional[TimeLike] = None,
        ...
        **kwargs: t.Any,
        ...
        """Same as `render_query_with_original_sql()` but raises an exception if the query can't be rendered.
        ...
        Returns:
            The rendered query along with original SQL.
        """
        result = self.render_query_with_original_sql(
            start=start,
            end=end,
            execution_time=execution_time,
            snapshots=snapshots,
    ...
    def render_audit_query(
        ...
        **kwargs: t.Any,
        ...
        # TODO: execute the original sql here as well?
    ...
    def _render_statements(
        ...
        **kwargs: t.Any,
        ...
        return [
            r
            for rendered_expressions in rendered_statements
            if rendered_expressions
            for r in rendered_expressions.rendered
        ]
    ...
    def render_signals(
        ...
        *,
        ...
        def _render(e: exp.Expression) -> str | int | float | bool:
            rendered_expressions = self._create_renderer(e).render(
                start=start, end=end, execution_time=execution_time
            )
            rendered_exprs = rendered_expressions.rendered if rendered_expressions else []
    ...
    def render_signal_calls(self) -> EvaluatableSignals:
        ...
        for name, kwargs in self.signals:
            ...
            for k, v in kwargs.items():
                rendered_expressions = self._create_renderer(v).render()
                rendered_exprs = rendered_expressions.rendered if rendered_expressions else []
                if len(rendered_exprs) != 1:
                    raise SQLMeshError(f"Expected one expression but got {len(rendered_exprs)}")
    ...
    def render_merge_filter(
        ...
        *,
        ...
        if self.merge_filter is None:
            return None
        rendered_expressions = self._create_renderer(self.merge_filter).render(
            start=start, end=end, execution_time=execution_time
        )
        rendered_exprs = rendered_expressions.rendered if rendered_expressions else []
        if len(rendered_exprs) != 1:
    ...
    def _render_properties(
        ...
        def _render(expression: exp.Expression) -> exp.Expression | None:
            ...
            rendered_expressions = self._statement_renderer(expression).render(**render_kwargs)
            rendered_exprs = rendered_expressions.rendered if rendered_expressions else []
    ...
    def ctas_query(self, **render_kwarg: t.Any) -> exp.Query:
        ...
        """
        rendered_query = self.render_query_or_raise_with_original_sql(**render_kwarg)

        original: t.Optional[str]
        query, original = rendered_query.rendered, rendered_query.original
        query = query.limit(0)
        ...
        if query.args.get("with") and (
            ...
            original = f"SELECT * FROM ({original}) AS _subquery LIMIT 0 WHERE FALSE"
        ...
        # return d.RenderedQuery(rendered=query, original=original)
        return query
    ...
    def _data_hash_values(self) -> t.List[str]:
        ...
        for statement in (*self.pre_statements, *self.post_statements):
            ...
            if not isinstance(statement, d.MacroDef):
                rendered_expressions = self._statement_renderer(statement).render()
                rendered_exprs = rendered_expressions.rendered if rendered_expressions else []

                if self._is_metadata_statement(statement):
                    continue
                if rendered_exprs:
                    statement_exprs = rendered_exprs
                else:
...
class SqlModel(_Model):
    ...
    def columns_to_types(self) -> t.Optional[t.Dict[str, exp.DataType]]:
        if self.columns_to_types_ is not None:
            ...
            try:
                rendered_query = self._query_renderer.render()
            except Exception:
...
def render_expression(
    ...
    rendered_expressions = ExpressionRenderer(
        expression,
        dialect,
        [],
        path=path,
    ...
    # Note: this is currently used for rendering model metadata, so the original SQL is unnecessary
    return rendered_expressions.rendered if rendered_expressions else None
