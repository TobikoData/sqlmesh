...
It empowers your data engineers and analysts to work with confidence by catching problems early as they work on
new features or make changes to your models.

# What exactly are audits
Audits are SQL queries that should not return any rows. In other words, they query for bad data, which indicate something
is wrong. In its simplest form, an audit is defined with the custom AUDIT expression along with a query.
```sql
AUDIT (
...
WHERE ds BETWEEN @start_ds AND @end_ds AND
   price IS NULL
```
In the above example, we defined an audit named `assert_item_price_is_not_null` on the model `sushi.items` to ensure every sushi
item has a price. If the query is in a different dialect than the rest of your project, you can specify it here like we did and
SQLGlot will automatically know how to execute the query. While the query can technically be on any model, or even multiple models,
the model specified in the audit definition tells SQLMesh when to run the audit during your pipeline's execution. If the query
returns any records, it means there might be an issue that needs your attention.

Audits are defined in `.sql` files in an `audit` directory in your SQLMesh project. Multiple audits can be defined in a single file
so you can organize them to your liking.

# Running audits
## The CLI audit command
You can execute audits with the `sqlmesh audit` command.
```
...
## Automated auditing
When you apply a plan, SQLMesh will automatically run each model's audits. By default, SQLMesh will halt the pipeline when an
audit fails in order to prevent potentially invalid data from propagating further downstream. This behvavior can be changed
for individual audits. See [Non-blocking audits](#non-blocking-audits)

# Advanced usage
## Skipping audits
Audits can be skipped by setting the skip argument to true.
```sql
AUDIT (
  name assert_item_price_is_not_null,
  model sushi.items,
...
## Non-blocking audits
By default, audits that fail will stop the execution of the pipeline in order to prevent bad data from propagating further
downstream. An audit can be configured to notify you when it fails without blocking the execution of the pipeline.
```sql
AUDIT (
  name assert_item_price_is_not_null,
  model sushi.items,
