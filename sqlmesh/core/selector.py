    ...
    def expand_model_selections(
        ...
        def evaluate(node: exp.Expression) -> t.Set[str]:
            ...
            if isinstance(node, exp.Anonymous):
                ...
                if modifier == "TAG":
                    pattern = node.expressions[0].name.lower()
                ...
                if modifier == "GIT":
                    target_branch = node.expressions[0].name
                    git_modified_files = {
                ...
                for model_name in evaluate(node.expressions[0]):
...
def parse(selector: str, dialect: DialectType = None) -> exp.Expression:
    ...
    def _parse_var() -> exp.Expression:
        ...
        if _match(TokenType.VAR):
            ...
            direction = "U" if upstream else ""
            direction += "D" if downstream else ""
        ...
        if tag:
            this = exp.func("TAG", this)
        if git:
            this = exp.func("GIT", this)
        if direction:
            this = exp.func(direction, this, copy=False)
        return this
