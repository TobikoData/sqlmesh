...
from sqlmesh.core.model import SqlModel, SeedModel, Model
...
import functools
from sqlmesh.dbt.converter.jinja_builtins import migrated_ref, migrated_source
...
def convert_project_files(
    src: Path, dest: Path, output_external_models: bool = True, no_prompts: bool = True
) -> None:
    console = DbtConversionConsole()
    report = ConversionReport()

    _fix_meta_field_converters()

    console.log_message(
        f"Converting project at '{src}' to '{dest}' (external models: {output_external_models})"
    )
    ...
    if output_external_models:
        ...
        if len(ctx.configs) == 1:
            # The SQLMesh context is currently tied to the source DBT project location
            # Update the config paths to be in the target SQLMesh project location so that create_external_models() writes the external models file in the right place
            ctx.configs = {dest: config for _, config in ctx.configs.items()}
            console.log_message("Creating external models file")
        ...
        console.log_message(f"[red]Skipping external models file due to config[/red]")
...
def _fix_meta_field_converters() -> None:
    from sqlmesh.core.model.definition import META_FIELD_CONVERTER
    ...
    # update the renderer to render 'depends_on ()' in this case to prevent a load error
    def _new_depends_on(value: t.Any) -> t.Optional[str]:
        if value is not None:
            if len(value) > 0:
                return orig_depends_on(value)
            return "()"
        return None

    META_FIELD_CONVERTER["depends_on_"] = _new_depends_on
...
def _convert_models(
    ...
    for idx, (model_name, model) in enumerate(ctx.models.items()):
        ...
        if model.kind.is_external:
            console.log_message(
                f"Skipping external model '{model.name}'; it will be written to `external_models.yaml` at the end"
            )
        ...
        # remove depends_on because DbtLoader sets it to fully qualified names and theyre a bit redundant once the {{ ref() }}'s are resolved
        # however, we still need to keep it for models where the entire query is a macro call, otherwise the SQLMesh native loader will throw:
        ...
        if isinstance(
            model, (SqlModel, SeedModel)
        ):  # note: should be no PythonModel's in a DBT project
            model_to_render = model.model_copy(
                update=dict(depends_on_=filtered_depends_on(ctx, model))
            )
            ...
            if isinstance(model, SqlModel):
                ...
                if is_self_referencing(model, test_rendered):
                    ...
                    output_columns = model.columns_to_types or {
                        "COLUMN_TYPES_CANNOT_BE_STATICALLY_ANALYZED": exp.DataType.build("unknown")
                    }
                    model_to_render = model_to_render.model_copy(
                        update=dict(columns_to_types_=output_columns)
                    )
...
def filtered_depends_on(ctx: Context, model: Model) -> t.Set[str]:
    ...
    return {
        ctx.models[i].name
        for i in model.depends_on
        if i in ctx.models and not ctx.models[i].kind.is_external
    }


def is_self_referencing(model: SqlModel, transformed_query: str) -> bool:
    """
    Check if a model is self referencing after transformation.

    If the self-reference is within an {% is_incremental() %} block then the DBT loader wont detect this.

    However, once the model is transformed to SQLMesh format, the {% is_incremental() %} blocks are removed which means
    the model may now be detected as self-referencing. We test render the transformed query to check if this is the case
    because if it is, the columns_to_types must all be known or the model will not load in the native loader
    """
    if model.depends_on_self:
        return True

    try:
        parsed = d.parse_one(transformed_query, dialect=model.dialect)
        if isinstance(parsed, d.Jinja):
            jinja_macros = model.jinja_macros.copy()
            api = jinja_macros._create_builtin_globals({})["api"]
            rendered = (
                jinja_macros.build_environment(
                    __migrated_ref=functools.partial(migrated_ref, api),
                    __migrated_source=functools.partial(migrated_source, api),
                )
                .from_string(parsed.name)
                .render()
            )
            parsed = d.parse_one(rendered, dialect=model.dialect)
        tables = d.find_tables(parsed, default_catalog=model.default_catalog, dialect=model.dialect)
        return model.fqn in tables
    except ParsetimeAdapterCallError:
        return model.depends_on_self
