...
def convert_jinja_query(
    ...
    exclude: t.Optional[t.List[t.Type[JinjaTransform]]] = None,
) -> t.Union[d.JinjaQuery, d.JinjaStatement, exp.Query]:
    ...
    transforms = [t for t in transforms if not any(e.__name__ in str(t) for e in (exclude or []))]
    ...
    if not _contains_jinja(post_processed):
        ...
        transformed = parsed.transform(st.unwrap_macros_in_string_literals())
        if isinstance(transformed, exp.Query):
            return transformed

        raise ValueError(f"Transformation resulted in a {type(transformed)} node instead of Query")
