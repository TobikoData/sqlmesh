...
_cache: t.Optional[ModelCache] = None
...
def _init_model_defaults(
    ...
    _context, _cache, _virtual_environment_mode, _audits = context, cache, virtual_environment_mode, audits


def _convert_dbt_model(dbt_model: BMC) -> t.List[Model]:
    assert _context
    assert _cache
    ...
    def _load_models(
        ...
        for project in self._load_projects():
            ...
            for package in project.packages.values():
                ...
                if unloaded:
                    chunksize = math.ceil(len(unloaded) / c.MAX_FORK_WORKERS)
