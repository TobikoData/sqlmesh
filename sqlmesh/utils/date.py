...
def to_time_column(
    ...
    # Clickhouse nullable types are nested, with the outer being Type.NULLABLE and inner being the actual data type.
    # We extract the inner type for the comparisons below.
    time_column_comparison_type = (
        time_column_type.expressions[0]
        if time_column_type.is_type(exp.DataType.Type.NULLABLE)
    ...
    if time_column_comparison_type.this in TEMPORAL_TZ_TYPES:
        return exp.cast(exp.Literal.string(to_tstz(time_column)), to=time_column_type)
    if time_column_comparison_type.this in exp.DataType.TEMPORAL_TYPES:
    ...
    if time_column_comparison_type.this in exp.DataType.TEXT_TYPES:
        return exp.Literal.string(time_column)
    if time_column_comparison_type.this in exp.DataType.NUMERIC_TYPES:
