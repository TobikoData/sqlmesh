...
MAX_TEXT_INDEX_LENGTH = 380


def index_text_type(dialect: DialectType) -> str:
    """
    MySQL and MSSQL cannot create indexes or primary keys on TEXT fields; they
    require that the fields have a VARCHAR type of fixed length. This helper
    simply abstracts away the type of such fields.
    """
    return f"VARCHAR({MAX_TEXT_INDEX_LENGTH})" if dialect in ("mysql", "tsql") else "TEXT"
