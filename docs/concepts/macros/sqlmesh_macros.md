...
Access global variable values in a model definition using the `@VAR()` macro function. The function requires the name of the variable _in single quotes_ as the first argument and an optional default value as the second argument. The default value is a safety mechanism used if the variable name is not found in the project configuration file.
...
A model definition could access the `int_var` value in a `WHERE` clause like this. Note that the variable name is in single quotes in the call `@VAR('int_var')`:
