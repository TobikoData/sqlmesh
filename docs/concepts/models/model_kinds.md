...
### When Matched Exclude Expression

The `when_matched_exclude` values can be column names to exclude from comparisons when using doing a merge on an INCREMENTAL_BY_UNIQUE_KEY model. This is only used on the default `when matched` clause generated by sqlmesh, where it has not been overridden by `when_matched`. The use of this keyword is for a [Date Loaded] field or similar, which will be inserted into the table, or updated in a row, but should not trigger an update of the row on its own.
...
MODEL (
  ...
  kind INCREMENTAL_BY_UNIQUE_KEY (
    unique_key COALESCE("name", ''),
    when_matched_exclude [Date Loaded]
  )
);
```

**Note**: This is only for engines that support the `MERGE` statement, see above.
