...
| `depends_on_past` | Whether or not each interval depends on the previous interval. If this is set to `True`, the model will run sequentially. If this is omitted, the model will run in parallel unless the model is `INCREMENTAL_BY_UNIQUE_KEY` or references itself. (Default: `None`)                                  | bool |    N     |                                                                                                                                                                                                                         | int  |    N     |
